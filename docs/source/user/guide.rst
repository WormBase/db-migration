...
Migration Procedure
===================
The :term:`azanium` commands are all required to be invoked from the same working
directory, and from the same host machine.
...
The first time you run the migration from a given UNIX user account,
you may want to setup slack notifications.

The url `$WEBHOOK_URL` is available from the WormBase slack management console for
the WormBase orgnaisation.
